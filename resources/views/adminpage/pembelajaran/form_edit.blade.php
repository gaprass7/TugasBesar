            ...
            <div class="col-lg-8">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <!-- Horizontal Form -->
                        <form method="POST" action="{{ route('pembelajaran.update', $row->id) }}">
                            @csrf
                            @method('PUT')
                                    ...
                                    <div class="input-group input-group-static mb-4">
                                        ...
                                        <div class="col-md-12">
                                            <select class="form-control" name="kursus_id">
                                                ...
                                                @foreach($ar_kursus as $item)
                                                @php
                                                    $sel = ($item->id == $row->kursus_id) ? 'selected' : '' ;
                                                @endphp
                                                <option value="{{ $item->id }}" {{ $sel }}> {{ $item->judul }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="input-group input-group-static mb-4">
                                        ...
                                        <div class="col-md-12">
                                            <select class="form-control" name="materi_id">
                                                ...
                                                @foreach($ar_materi as $items)
                                                @php
                                                    $sel = ($items->id == $row->materi_id) ? 'selected' : '' ;
                                                @endphp
                                                <option value="{{ $items->id }}" {{ $sel }}>{{ $items->judul }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
