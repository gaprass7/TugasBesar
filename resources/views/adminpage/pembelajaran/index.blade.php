                    ...
                    <table class="table align-items-center mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-uppercase font-weight-bolder opacity-7 ps-2">Judul</th>
                                <th class="text-center text-uppercase font-weight-bolder opacity-7">Deskripsi</th>
                                <th class="text-center text-uppercase font-weight-bolder opacity-7">Materi</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                    <img src="{{ url('admin/img') }}/{{ $row->kursus->foto }}" width="55px" alt="Profile" class="rounded-circle">
                                    @endempty
                                </td>

                                <td><p class="text-xs font-weight-bold mb-0">{{ $row->kursus->judul }}</p></td>
                                <td><p class="text-xs font-weight-bold mb-0">{{ $row->kursus->deskripsi }}</p></td>
                                <td><p class="text-xs font-weight-bold mb-0">{{ $row->materi->judul }}</p></td>
                                {{-- <td><p class="text-xs font-weight-bold mb-0">{{ $row->materi->deskripsi }}</p></td> --}}
                                <td class="align-middle text-center">
                                    <span class="text-secondary text-xs font-weight-bold">{{ $row->kursus->durasi }}</span>
                                </td>

                                <td>
                                    <form method="POST" id="formDelete" action="{{ route('pembelajaran.destroy', $row->kursus->id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <a class="btn btn-info btn-sm" title="Detail Kursus"
                                            href="{{ route('pembelajaran.show', $row->kursus->id) }}">
                                            <i class="bi bi-eye">Show</i>
                                        </a>
                                        <a class="btn btn-warning btn-sm" title="Ubah Kursus"
                                            href="{{ route('pembelajaran.edit', $row->kursus->id) }}">
                                            <i class="bi bi-pencil">Edit</i>
                                        </a>
                                        <button type="submit" class="btn btn-danger btn-sm btnDelete" title="Hapus Kursus">
